    ...
    describe("AssignmentQueryUseCase") {
        ...
        context("완료된 과제 존재 여부 확인") {
            ...
            it("완료된 과제가 있으면 true를 반환한다") {
                every {
                    assignmentRepository.existsByMentee_IdAndStatusNot(
                        menteeId,
                        AssignmentStatus.NOT_SUBMIT
                    )
                } returns true
        ...
        context("학습 시간 및 개수 조회") {
            val menteeId = 1L

            it("전체 학습 시간을 조회한다") {
                every { assignmentRepository.sumStudyTimeByMenteeId(menteeId, null) } returns 120L
                useCase.getTotalStudyTimeBySubject(menteeId) shouldBe 120L
            }

            it("25분 이상 공부한 과제 개수를 조회한다") {
                every { assignmentRepository.countCompletedOver25Minutes(menteeId) } returns 5L
                useCase.getCompletedOver25MinutesCount(menteeId) shouldBe 5L
            }

            it("6시~9시 사이 공부한 과제 개수를 조회한다") {
                every { assignmentRepository.countStudyBetweenSixAndNine(menteeId) } returns 3L
                useCase.getStudyBetweenSixAndNineCount(menteeId) shouldBe 3L
            }
        }
    }
})
