...
data class AssignmentTemplateUpdateReq(
    ...
    @field:NotBlank
    @Schema(description = "과제 내용", example = "문제 1번 풀이", requiredMode = Schema.RequiredMode.REQUIRED)
    val content: String,

    @Schema(description = "템플릿에 포함할 학습자료 ID 목록 (전체 교체). null이면 변경 없음", example = "[1, 2, 3]", requiredMode = Schema.RequiredMode.NOT_REQUIRED)
    val learningResourceIds: List<Long>? = null
)